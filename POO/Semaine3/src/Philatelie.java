...
class Rare extends Timbre{
    ...
    @Override
    public double vente() {
        double prixDeBbase = 0;
        if(getExemplaires() >0 && getExemplaires() < BASE_1_EXEMPLAIRES){
            prixDeBbase = PRIX_BASE_1;
        }else if(getExemplaires() < BASE_2_EXEMPLAIRES){
            prixDeBbase = PRIX_BASE_2;
        }else{
            prixDeBbase = PRIX_BASE_3;
        }
        return prixDeBbase * (age() / 10.0);
    }
}
